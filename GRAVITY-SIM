...
class Planet(vp.sphere):
    ...
    def __init__(self):
        super().__init__(
            ...
            # Variables below aren't keywords in vpython
    ...
    def update(self, planets_list):
        """Updates position and velocity of planet'planets_list' should be a
        copy of planets_list that is used to remove the bias associated with
        updating the first elements of the planets list before updating the
        following elements.
        """
        if updates != True:
            return None
    ...
    @staticmethod
    def update_all(planets_list):
        """Iterates through all planets and updates positions."""
        planets_copy = planets_list  # Used to remove bias from positioning within planets
        for planet in planets_list:
            planet.update(planets_copy)
        return planets_list

    @staticmethod
    def check_for_collisions(planets_list):
        ...
        for planet in planets_list:
            for other in planets_list:
                ...
                if dist <= planet.radius + other.radius + max_proximity:
                    ...
                    planets_list.remove(other)

                    del planet
                    del other

                    planets_list.append(new_planet)
                    return Planet.check_for_collisions(planets_list) # Recursion!
